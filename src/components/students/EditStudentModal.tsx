...
export function EditStudentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed || trimmed.length < 2) {
      setError("O'quvchi ism va familiyasini to'liq kiriting");
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "O'quvchini tahrirlashda xatolik");
      }

      onStudentUpdated({ ...student, ...data.student });
      onClose();
    } catch (err: any) {
      setError(err.message || "O'quvchi saqlanmadi. Qayta urinib ko'ring.");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white relative">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold">O‘quvchini tahrirlash</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              O'quvchi ism-familiyasini o'zgartiring
            </p>
          </div>
        </div>
