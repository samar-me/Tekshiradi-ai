  ...
  const handleDelete = async () => {
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "O'quvchini o'chirishda xatolik");
      }

      onStudentDeleted(student.id);
      onClose();
    } catch (err: any) {
      setError(err.message || "O'quvchi o'chirilmadi. Qayta urinib ko'ring.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-6 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-center">
        ...
        <h3 className="text-base font-bold">O‘quvchini o‘chirmoqchimisiz?</h3>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
          <strong className="text-slate-800 dark:text-slate-200">{student.full_name}</strong> ro‘yxatdan o‘chiriladi. Ushbu amalni bekor qilib bo‘lmaydi.
        </p>
